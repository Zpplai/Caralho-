...
modalContent.innerHTML =   <div class="relative bg-slate-900 rounded-3xl overflow-hidden">   <button id="closeGameModal" class="absolute top-4 right-4 bg-black/50 text-white w-10 h-10 rounded-full z-10">✕</button>   <div class="bg-[#020617] flex justify-center p-6">   <img src="${linkImg}" class="h-64 object-contain">   </div>   <div class="p-8">   <h2 class="text-3xl font-black text-white uppercase italic">${j.nome}</h2>   <p class="text-slate-400 my-6 text-sm leading-relaxed">${j.desc}</p>   <a href="${j.link}" target="_blank" class="block w-full bg-blue-600 text-center py-5 rounded-2xl font-black text-white uppercase shadow-lg hover:bg-blue-500 transition-all">   BAIXAR AGORA   </a>   </div>   </div>  ;
...
Aqui o código completo consegue fazer pra mim por favor e pq eu apaguei o arquivo meu e eu perdi a sequência
