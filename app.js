...
window.onload = () => {
  ...
  // 11 - Carregar jogos do Firestore
  ...
  function renderGames(filtro) {
      ...
      jogosCache.forEach(j => {
          ...
          card.innerHTML = `
              <div class="relative overflow-hidden rounded-xl mb-3 bg-[#020617] h-60 w-full flex items-center justify-center">
                  <img src="${linkImg}" onerror="this.src='${imgPadrao}'" class="w-full h-full object-cover transition-all">
              </div>
  ...
  // 17 - Fechar modal clicando fora
  if (modal) modal.onclick = (e) => { if (e.target.id === 'gameModal') modal.classList.add('hidden'); };
  ...
  // 19 - Abrir modal de adicionar jogo
  const openAddModal = document.getElementById('openAddModal');
  ...
  // 20 - Cancelar modal admin
  const cancelBtn = document.getElementById('cancelBtn');
  if (cancelBtn) cancelBtn.onclick = () => document.getElementById('adminModal').classList.add('hidden');

  // 21 - Formulário de adicionar/editar jogo
  const gameForm = document.getElementById('gameForm');
  if (gameForm) gameForm.onsubmit = async (e) => {
      e.preventDefault();
      const data = {
